...
//                and the MQTT Application via the ESP8266. 
//
...
char    eid[32]           = "";         // Your EID goes here
char    ssid[32]          = "";         // SSID for the WAP you are using
char    pass[32]          = "";         // Password for the access to the WAP
char    mqtt_broker[32]   = "";         // IP Address to your Broker
char    mqtt_port[8]      = "1883";     // Port for TCP/IP access to the Broker
...
int Tx5Fifo_Put(char data)
{
  ...
  Tx5Fifo[Tx5PutI&(TX5FIFOSIZE-1)] = data; // put data in the FIFO
...
void UART5_Init(uint32_t priority)
{
  ...
  NVIC_PRI15_R = (NVIC_PRI15_R & 0xFFFF00FF)
                               | (priority<<13); // bits 13-15
...
void ESP8266_SetupWiFi(void) { 
  ...
  ESP8266_OutString(eid);           // Student EID - Used for individualizing MQTT Topics 
  ESP8266_OutChar(',');
  ESP8266_OutString(ssid);          // Send WiFi SSID to ESP8266
  ESP8266_OutChar(',');
  ESP8266_OutString(pass);          // Send WiFi Password to ESP8266
  ESP8266_OutChar(',');
  ESP8266_OutString(mqtt_broker);   // Send IP address of MQTT Broker
  ESP8266_OutChar(',');
  ESP8266_OutString(mqtt_port);     // Send MQTT port number
